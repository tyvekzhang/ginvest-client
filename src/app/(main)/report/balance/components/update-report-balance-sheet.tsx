...
import { ReportBalanceSheet } from '@/types/report-balance-sheet';
import { UpdateReportBalanceSheet } from '@/types/report-balance-sheet';
import { FormInstance } from 'antd/es/form';
import React, { useMemo } from 'react';

interface UpdateReportBalanceSheetProps {
  isUpdateReportBalanceSheetModalVisible: boolean;
  onUpdateReportBalanceSheetCancel: () => void;
  onUpdateReportBalanceSheetFinish: () => void;
  isUpdateReportBalanceSheetLoading: boolean;
  updateReportBalanceSheetForm: FormInstance<UpdateReportBalanceSheet>;
  treeSelectDataSource?: ReportBalanceSheet[];
}

const updateReportBalanceSheetFormItemLayout = {
  labelCol: { span: 6 },
  wrapperCol: { span: 18 },
};

const UpdateReportBalanceSheetComponent: React.FC<UpdateReportBalanceSheetProps> = ({
  isUpdateReportBalanceSheetModalVisible,
  onUpdateReportBalanceSheetCancel,
  onUpdateReportBalanceSheetFinish,
  isUpdateReportBalanceSheetLoading,
  updateReportBalanceSheetForm,
  ...
  const footerButtons = useMemo(
    () => [
      <Button key="cancel" onClick={onUpdateReportBalanceSheetCancel}>
        取消
      </Button>,
      <Button key="submit" type="primary" loading={isUpdateReportBalanceSheetLoading} onClick={onUpdateReportBalanceSheetFinish}>
        确定
      </Button>,
    ],
    [isUpdateReportBalanceSheetLoading, onUpdateReportBalanceSheetCancel],
  );

  return (
    <Modal
      title="资产负债编辑"
      open={isUpdateReportBalanceSheetModalVisible}
      onCancel={onUpdateReportBalanceSheetCancel}
      footer={footerButtons}
      destroyOnHidden
      width={"60%"}
    >
        <Form
          {...updateReportBalanceSheetFormItemLayout}
          form={ updateReportBalanceSheetForm}
          name="updateReportBalanceSheet"
          onFinish={onUpdateReportBalanceSheetFinish}
...
export default UpdateReportBalanceSheetComponent;