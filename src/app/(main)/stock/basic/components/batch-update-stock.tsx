...
import { BatchUpdateStock } from '@/types/stock';
import { FormInstance } from 'antd/es/form';
import React, { useMemo } from 'react';


interface BatchUpdateStocksProps {
  isBatchUpdateStocksModalVisible: boolean;
  onBatchUpdateStocksCancel: () => void;
  onBatchUpdateStocksFinish: () => void;
  isBatchUpdateStocksLoading: boolean;
  batchUpdateStocksForm: FormInstance<BatchUpdateStock>;
}

const batchUpdateStocksFormItemLayout = {
  labelCol: { span: 6 },
  wrapperCol: { span: 18 },
};

const BatchUpdateStocksComponent: React.FC<BatchUpdateStocksProps> = ({
  isBatchUpdateStocksModalVisible,
  onBatchUpdateStocksCancel,
  onBatchUpdateStocksFinish,
  isBatchUpdateStocksLoading,
  batchUpdateStocksForm,
}) => {
  
  const footerButtons = useMemo(
    () => [
      <Button key="cancel" onClick={onBatchUpdateStocksCancel}>
        取消
      </Button>,
      <Button key="submit" type="primary" loading={isBatchUpdateStocksLoading} onClick={onBatchUpdateStocksFinish}>
        确定
      </Button>,
    ],
    [isBatchUpdateStocksLoading, onBatchUpdateStocksCancel],
  );

  return (
    <Modal
      title="股票基础信息批量编辑"
      open={isBatchUpdateStocksModalVisible}
      onCancel={onBatchUpdateStocksCancel}
      footer={footerButtons}
      destroyOnHidden
      width={"60%"}
    >
        <Form
          {...batchUpdateStocksFormItemLayout}
          form={ batchUpdateStocksForm}
          name="batchUpdateStocks"
          onFinish={onBatchUpdateStocksFinish}
...
export default BatchUpdateStocksComponent;